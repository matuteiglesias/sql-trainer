...
Until the dependency-verification recovery PR is complete, Vite serves the
existing frozen bundle tree (exposed at `fixtures/` by a text-only symbolic link to the original bundle) via `publicDir: "fixtures/target/public"`. Runtime
URLs remain `/db/hospital_v0_1.sqlite` and `/content/exercises/index.json`; the
database and exercise corpus are not duplicated at the repository root.
